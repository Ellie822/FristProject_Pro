...
public class ServerReaderThread extends Thread {
            ...
            while (true) {
                ...
                switch (type) {
                    ...
                    case 3:
                        ...
                        // 先从socket管道中接收私聊目标，再接收私聊消息内容
                        String target = dis.readUTF();//目标用户名
                        String msg1v1 = dis.readUTF();//私聊消息内容
                        pm(target, msg1v1);
                        break;
                }
            }
    ...
    private void pm(String target, String msg1v1) {
        String sender = Server.onLineSockets.get(socket);// 发送者用户名
        StringBuilder sb = new StringBuilder();
        for (Socket socket : Server.onLineSockets.keySet()) {//遍历所有在线socket
            try {
                DataOutputStream dos = null;//获取目标socket的输入流
                dos = new DataOutputStream(socket.getOutputStream());
                if (Server.onLineSockets.get(socket).equals(target)) {
                    //匹配目标用户名,给目标发送消息
                    String msg = sb.append(sender).append(" to ").append(target).append(":").append(msg1v1).toString();
                    dos.writeInt(3);
                    dos.writeUTF(msg);
                    dos.flush();
                    //除了给目标发送消息，发送的消息发送者自己也要能看见
                    DataOutputStream dos_sender=new DataOutputStream(this.socket.getOutputStream());
                    dos_sender.writeInt(3);
                    dos_sender.writeUTF(msg);
                    dos_sender.flush();
                    return;
                }
            } catch (IOException e) {
        ...
        try {
            DataOutputStream dos=new DataOutputStream(socket.getOutputStream());//要是目标socket不存在 ，就给发送者发送消息：“找不到该用户”
            dos.writeInt(3);
            dos.writeUTF("找不到该用户\r\n");
            dos.flush();
        } catch (IOException e) {
