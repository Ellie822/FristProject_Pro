...
public class ClientChatFrame extends JFrame {
    ...
    private JButton privateChatBn = new JButton("私聊");
    ...
    private void initView() {
        ...
        // 私聊按钮
        privateChatBn.setFont(font);
        privateChatBn.setBackground(Color.decode("#2196F3")); // 蓝色背景区别于发送按钮
        privateChatBn.setForeground(Color.WHITE);

        // 按钮面板
        JPanel btns = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 5));
        btns.setBackground(new Color(0xf0, 0xf0, 0xf0));
        btns.add(sendBn);
        btns.add(privateChatBn);
        ...
        // 给私聊按钮绑定点击事件
        privateChatBn.addActionListener(e -> {
            // 获取输入框中的内容
            String msg = smsSend.getText();
            // 清空输入框
            smsSend.setText("");
            sendMsgToServer_private(msg);//给服务器发送私聊需要的信息
        });
    ...
    private void sendMsgToServer_private(String msg) {
        try {
            DataOutputStream dos = new DataOutputStream(socket.getOutputStream());
            dos.writeInt(3);//表示是私聊
            String  target=getTaragetName(msg);//获取目标用户名
            String trueMessage=getTrueMsg(msg);//真正的消息
            dos.writeUTF(target);//使用管道发送目标用户名
            dos.writeUTF(trueMessage);//使用管道发送消息
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private String getTrueMsg(String msg) {
        int spaceIndex = msg.indexOf(' ');
        // 若没有空格，返回空字符串；否则从空格后一位开始截取
        return spaceIndex == -1 ? "" : msg.substring(spaceIndex + 1);
    }

    private String getTaragetName(String msg) {
        int spaceIndex = msg.indexOf(' ');
        return spaceIndex == -1 ? msg : msg.substring(0, spaceIndex);
    }
